...
setup_master_node() {
    ...
    mkdir -p $USER_HOME/tethys-v-c3
    cp -f /etc/kubernetes/admin.conf $USER_HOME/tethys-v-c3/config
    chown -R $NORMAL_USER:$NORMAL_USER $USER_HOME/tethys-v-c3

    echo "마스터 노드 kubeconfig를 공유 스토리지에 복사했습니다: ~/tethys-v-c3/config"
...
setup_worker_node() {
    ...
    # tethys-v-c3는 이미 공유 스토리지로 마운트되어 있다고 가정
    mkdir -p $HOME/.kube
    cp ~/tethys-v-c3/config ~/.kube/config
