...
wait_for_resources_or_arrival() {
    ...
    while true; do
        ...
        if [ $TOTAL_RESOURCES_USED -le $((16 - WORKER_NUM)) ]; then
            echo "Resources available for job ${JOB_NAME}. Starting now."
            return 0
        fi

        # 완료된 작업 확인 및 정리 (기존 코드와 동일)
        ...
        echo "Arrival time reached, waiting for resources to become available for job ${JOB_NAME}"
        sleep 0.1s
    done
}
